    ...
    return (
        ...
        >
            ...
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-20 h-6 bg-white/40 rounded-xl shadow flex items-center justify-center border border-gray-200 sm:w-24 sm:h-7 lg:w-32 lg:h-8">
                <Image src={`/mongolian-esports-awards${sportImg}`} alt="sportsImg" width={64} height={18} className="object-contain sm:w-[80px] sm:h-[20px] lg:w-[110px] lg:h-[24px]" priority />
            </div>
            {/* Logo */}
            <div className="relative flex items-center justify-center rounded-2xl mt-4 mb-2 w-24 h-24 lg:w-32 lg:h-32">
                <Image src={`/mongolian-esports-awards${img}`} alt="img" fill className="object-contain p-1 sm:p-2" priority />
            </div>
            {/* Team name */}
            <div className="w-full h-7 bg-white/30 rounded-xl flex items-center justify-center font-bold text-gray-800 tracking-wide border border-gray-200 text-[10px] sm:text-sm lg:text-[14px] lg:h-10">
                {name}
            </div>
        </motion.div>
    );
